...
function generateWindowsNativeFiles(options = {}) {
  ...
  console.log("")
  console.log(
    `${colors.D}Running ./bin/generate_windows_native_files.js from ${__filename}${colors.X}`,
  )
  console.log("")
...
function verifyOptions(options) {
  if (!options.appPath) {
    throw new Error(
      `${colors.RB}generate_windows_native_files - You must specify an appPath${colors.X}`,
    )
  }
}
...
function checkAppPath(appPath) {
  ...
  console.log("")
  console.log(
    `${colors.Y}  No files found in ${colors.BB}${appPath}${colors.X}${colors.Y}.${colors.X}`,
  )
...
function findWindowsNativeFiles(appPath, projectRoot) {
  ...
  moduleMap.forEach((module, name) => {
    if (module.files.h && module.files.cpp) {
      ...
      console.log(
        `${colors.BB} ✓ Found     ${colors.X} ${colors.BB}${name}${colors.X} ${colors.DD}${relativeDir}${colors.X} ${typeIndicator} ${module.type}`,
      )
    } else {
      console.log(
...
function generateConsolidatedFiles(modules, windowsDir, projectRoot) {
  ...
  let headerNeedsUpdate =
    !fs.existsSync(headerPath) || fs.readFileSync(headerPath, "utf8") !== headerContent
  ...
  if (cppContent) {
    ...
    const relativePath = path.relative(projectRoot, cppPath)
    console.log(
      `${colors.YB} ➖ Removed    ${colors.X}${colors.S}${colors.D}${relativePath}${colors.X}${colors.D} (no Fabric components)${colors.X}`,
    )
    changesMade = true
  }
}
...
function generateHeaderTemplate(modules, windowsDir) {
  ...
  const fabricRegistrations = fabricComponents
    .map((module) => {
      ...
      const functionName = registerMatch
        ? registerMatch[1]
        : `Register${module.name}NativeComponent`
...
function generateCppTemplate(modules) {
  ...
  const fabricRegistrationCalls = fabricComponents
    .map((module) => {
      // Extract registration function name from header content
      const headerContent = fs.readFileSync(module.files.h, "utf8")
      const registerMatch = headerContent.match(/void\s+(Register\w*NativeComponent)\s*\(/)
...
function cleanGeneratedFiles(windowsDir, projectRoot) {
  const filesToClean = ["IRNativeModules.g.h", "IRNativeModules.g.cpp"]
  ...
  filesToClean.forEach((file) => {
    ...
    if (fs.existsSync(filePath)) {
      ...
      console.log(
        `${colors.YB} ➖ Removed    ${colors.X}${colors.S}${colors.D}${relativePath}${colors.X}`,
      )
      changesMade = true
    }
  })
